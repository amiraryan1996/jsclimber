...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Error fetching posts' }, { status: 400 });
  }
}

// Add a new post
export async function POST(req: Request) {
  const { name, description, imageUrl, articleUrl, slug, date, categoryId, summary } =
    await req.json();
  ...
  const query = `
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?);
  `;
  const values = [name, description, imageUrl, articleUrl, slug, date, categoryId, summary];
